...
let cardData={};
async function loadCardData(){try{const r=await fetch(`data/cards.json?v=${Date.now()}`,{cache:'no-store'});if(!r.ok)throw Error(r.status);cardData=await r.json()}catch(e){console.warn('卡片資料載入失敗',e);cardData={}}}
...
function renderCards(s,cards){emptyState.hidden=true;cardsGrid.hidden=false;cardsGrid.innerHTML='';if(!cards.length){cardsGrid.innerHTML=`<div class="empty-state" style="grid-column:1/-1"><div class="empty-icon">📷</div><h2>${s.id} 尚未抓到官方卡圖</h2><p>資料更新完成後重新整理即可看到卡片。</p></div>`;return}cards.forEach(c=>{const f=document.createElement('figure');f.className='card';const img=document.createElement('img');img.src=c.localImage||c.image;img.alt=c.id;img.loading='lazy';img.onerror=()=>{if(c.localImage&&img.src.endsWith(c.localImage)){img.src=c.image||'';return}img.classList.add('image-error');img.alt=`${c.id} 圖片載入失敗`;};f.appendChild(img);const cap=document.createElement('figcaption');cap.textContent=c.name?`${c.id}｜${c.name}`:c.id;f.appendChild(cap);f.onclick=()=>openLightbox(c);cardsGrid.appendChild(f)})}
function openLightbox(c){lightboxImage.src=c.localImage||c.image;lightboxImage.alt=c.id;lightboxImage.onerror=()=>{lightboxImage.src=c.image||''};lightboxLabel.textContent=c.name?`${c.id}｜${c.name}`:c.id;lightbox.hidden=false}function closeLightbox(){lightbox.hidden=true}
