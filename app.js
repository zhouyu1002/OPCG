...
let cardData={};
async function loadCardData(){try{const r=await fetch(`data/cards.json?v=${Date.now()}`,{cache:'no-store'});if(!r.ok)throw Error(r.status);cardData=await r.json()}catch(e){console.warn('官方卡圖資料尚未抓取',e);cardData={}}}
...
function renderCards(s,cards){emptyState.hidden=true;cardsGrid.hidden=false;cardsGrid.innerHTML='';if(!cards.length){cardsGrid.innerHTML=`<div class="empty-state" style="grid-column:1/-1"><div class="empty-icon">📷</div><h2>${s.id} 尚未抓到官方卡圖</h2><p>官方卡表抓取完成後，重新整理即可看到卡片。</p></div>`;return}cards.forEach(c=>{const f=document.createElement('figure');f.className='card';const img=document.createElement('img');img.src=c.image;img.alt=c.id;img.loading='lazy';img.referrerPolicy='no-referrer';img.onerror=()=>{img.classList.add('image-error');img.alt=`${c.id} 圖片載入失敗`;};f.appendChild(img);const cap=document.createElement('figcaption');cap.textContent=c.name?`${c.id}｜${c.name}`:c.id;f.appendChild(cap);f.onclick=()=>openLightbox(c);cardsGrid.appendChild(f)})}
function openLightbox(c){lightboxImage.src=c.image;lightboxImage.alt=c.id;lightboxImage.referrerPolicy='no-referrer';lightboxLabel.textContent=c.name?`${c.id}｜${c.name}`:c.id;lightbox.hidden=false}function closeLightbox(){lightbox.hidden=true}
